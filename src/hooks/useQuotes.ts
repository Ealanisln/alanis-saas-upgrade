...
export const useQuotes = (options: UseQuotesOptions = {}) => {
  ...
  const quotesQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (options.clientType)
        searchParams.set("clientType", options.clientType);

      const _url = `/quotes${searchParams.toString() ? `?${searchParams.toString()}` : ""}`;
  ...
  const createQuoteMutation = useMutation({
    mutationFn: async (request: QuoteRequest): Promise<QuoteResponse> => {
      const response = await client.post<ApiResponse<QuoteResponse>>(
        "/quotes",
        {
  ...
  return {
    ...
    error:
      ...
      (createQuoteMutation.error instanceof Error
        ...
        : null) ||
      (syncWithInvoiceNinjaMutation.error instanceof Error
